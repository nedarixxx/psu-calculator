...
- **Сравнение нескольких сборок по энергопотреблению** (опционально)
...
- `peak_power` (IntegerField, blank=True, null=True): Пиковое потребление
...
### Сценарий 0: Регистрация и вход
1. Гость переходит на главную страницу
2. Кликает "Зарегистрироваться"
3. Заполняет форму: email, пароль, подтверждение
4. Система создает аккаунт и перенаправляет на login
5. Пользователь вводит email и пароль
6. Система перенаправляет на личный кабинет

### Сценарий 0.5: Просмотр каталога (для гостей и пользователей)
1. Пользователь переходит в "Каталог компонентов"
2. Видит компоненты, отсортированные по категориям
3. Может использовать поиск и фильтры
4. Видит для каждого компонента: название, TDP (в Ваттах), цену
...
- [x] Модели (Category, Component, Build)
- [x] Формы и валидация
- [x] Аутентификация пользователей
- [x] Расчет энергопотребления с коэффициентом 1.25
- [x] Pie Chart через Matplotlib
- [x] Админ-панель с поиском и фильтрацией
- [x] Поиск и фильтрация в каталоге

### Планируется:
- [ ] Экспорт результатов в PDF
- [ ] Сравнение нескольких сборок
- [ ] История изменений сборок

### Известные ограничения:
- Экспорт в PDF может быть реализован на более позднем этапе (низкий приоритет)